...
func (r *Repository) GetByID(ctx context.Context, id uuid.UUID) (*domain.RestaurantAggregate, error) {
	row := sqltx.From(ctx, r.pool).QueryRow(ctx, `SELECT `+cols+`, `+policyCols+`, `+preorderCols+`, `+serviceFeeCols+` FROM restaurants WHERE id=$1`, id)
...
// ListManageableBrief returns a lightweight (id, name, name_i18n, is_active) row
// for EVERY restaurant, ordered by name. It backs the superadmin variant of
...
// catalog reads) — IsActive is carried through so the picker can label them,
// not to filter them out. Returns an empty slice, not an error, when there are
// none.
func (r *Repository) ListManageableBrief(ctx context.Context) ([]domain.RestaurantBrief, error) {
	rows, err := sqltx.From(ctx, r.pool).Query(ctx,
		`SELECT id, name, name_i18n, is_active FROM restaurants ORDER BY name`)
	if err != nil {
	...
	for rows.Next() {
		...
		var nameI18n []byte
		if err := rows.Scan(&b.ID, &b.Name, &nameI18n, &b.IsActive); err != nil {
			return nil, err
		}

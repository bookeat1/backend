...
const selCols = `id, restaurant_id, name, name_i18n, description, description_i18n,
	...
	created_at, updated_at, kwaaka_product_id`
...
// UpsertFromKwaaka inserts or updates one dish synced from Kwaaka, matching on
// (restaurant_id, kwaaka_product_id) — see migration 0114's partial unique
// index. On insert m.ID (generated by the caller) becomes the row's id; on
// conflict the EXISTING id is kept (excluded.id is never written), so the
// dish's FKs (tags, top_pick_position, is_featured) survive every sync. Only
...
func (r *Repository) UpsertFromKwaaka(ctx context.Context, m *domain.MenuItem) error {
	...
	m.UpdatedAt = now
	q := `INSERT INTO menu_items (id, restaurant_id, name, name_i18n, description, description_i18n,
			price, image_url, is_available, category, category_i18n, language, kwaaka_product_id,
			created_at, updated_at)
		...
		RETURNING id`
	args := []any{
		m.ID, m.RestaurantID, m.Name, i18nToDB(m.NameI18n), m.Description, i18nToDB(m.DescriptionI18n),
		m.Price, m.ImageURL, m.IsAvailable, m.Category, i18nToDB(m.CategoryI18n), m.Language,
		m.KwaakaProductID, m.CreatedAt, m.UpdatedAt,
	}
	row := sqltx.From(ctx, r.pool).QueryRow(ctx, q, args...)
	if err := row.Scan(&m.ID); err != nil {
		return mapWrite(err, "upsert menu item from kwaaka")
	}
	return nil
}

// MarkUnavailableExceptKwaakaIDs stop-lists every Kwaaka-sourced dish of
// restaurantID whose kwaaka_product_id fell out of the latest sync pass. A
// nil/empty keep still runs the UPDATE (WHERE kwaaka_product_id <> ALL($2)
// with an empty array matches everything) — see the domain method's doc
// comment for why that is correct, not a bug.
func (r *Repository) MarkUnavailableExceptKwaakaIDs(ctx context.Context, restaurantID uuid.UUID, keep []string) (int, error) {
	if keep == nil {
		// A NULL array parameter makes `x = ANY($2)` evaluate to NULL (not
		// false) for every row, which would make the NOT(...) predicate NULL
		// too and silently update NOTHING — the opposite of "mark everything
		// unavailable" that an empty keep is supposed to mean.
		keep = []string{}
	}
	tag, err := sqltx.From(ctx, r.pool).Exec(ctx,
		`UPDATE menu_items SET is_available=false, updated_at=now()
		 WHERE restaurant_id=$1 AND kwaaka_product_id IS NOT NULL
		   AND is_available = true
		   AND NOT (kwaaka_product_id = ANY($2))`,
		restaurantID, keep)
	if err != nil {
		return 0, fmt.Errorf("mark unavailable except kwaaka ids: %w", err)
	}
	return int(tag.RowsAffected()), nil
}
